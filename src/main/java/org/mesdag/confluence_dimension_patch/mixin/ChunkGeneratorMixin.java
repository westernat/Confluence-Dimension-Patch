...
public abstract class ChunkGeneratorMixin {
    ...
    private boolean skip(boolean original, @Local(argsOnly = true) ChunkGeneratorStructureState structureState, @Local(argsOnly = true) Holder<StructureSet> holder) {
        return original && (IDimensionAccessor.of(structureState).confluence_dimension_patch$isNotOverworld() || IStructureSet.of(holder.value()).confluence_dimension_patch$isNotFromConfluence());
    }
}
